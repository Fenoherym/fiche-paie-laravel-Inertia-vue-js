...
export default {
    ...
    data(){
        return{
            form:{
                numero:this.rubrique.numero,
                designation:this.rubrique.designation,
                base: this.rubrique.base != null ?  this.rubrique.base: '',
                taux_part_salarial: this.rubrique.taux_part_salarial,
                taux_part_patronal: this.rubrique.taux_part_patronal,
                type: this.rubrique.type,
                isRetenue: this.rubrique.isRetenue,
                isImposable: this.rubrique.isImposable
    ...
    methods:{
        ...
        postRubrique(){
            ...
            let url = `/rubrique/update/${this.rubrique.id}`
            Inertia.post(url, formData)
        }
    },
    created(){
        if(this.form.isRetenue===0){
            this.isRetenue=false
        }else{
            this.isRetenue=true
        }
        if(this.form.taux_part_patronal===null){
            this.form.taux_part_patronal= ''
        }
        if(this.form.taux_part_salarial===null){
            this.form.taux_part_salarial= ''
        }
    }
}
</script>
